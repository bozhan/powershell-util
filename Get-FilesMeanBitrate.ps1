...
function Get-MeanAttribute($file, [string]$attrName){
...
function Get-MeanAttributesForFiles([string]$folderPath, [int]$mbr = "$maxBitrate", [boolean]$recurse = $r){
	...
	if($files.count -gt 0){
		foreach($file in $files){
			...
			if($totalBr -ge $mbr) {
				$fTotalBr = "{0:N0}" -f $totalBr
				$fTotalABr = "{0:N0}" -f $totalABr
				$fTotalFw = "{0:N0}" -f $totalFw
				if($csv){
					write-host("{0},{1},{2},{3}" -f $fTotalBr, $fTotalFw, $fTotalABr, $file.Name)
				}else{
					write-host("{0,-5} {1, -5} {2, -5} {3}" -f $fTotalBr, $fTotalFw, $fTotalABr, $file.Name)
				}
			}
		}
	}else{
